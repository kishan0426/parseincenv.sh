...
# --- Pass 1: Capture Call Stack Trace & Highlight Error Functions ---
...
# --- Pass 2: Capture Compilation Environment Dump ---
    ...
    for (c = 1; c <= 6; c++) {
        ...
        if (count[cat_name] > 0) {
            ...
            for (i = 1; i <= count[cat_name]; i++) {
                ...
                # Enclose parameter name in double quotes as requested (e.g., "_px_broadcast_fudge_factor")
                quoted_p = "\"" p "\""
