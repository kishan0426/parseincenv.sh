...
# --- Pass 0.3: Capture Process Resource Limits ---
...
# --- Pass 0.4: Capture Process cgroup Information ---
index($0, "Dumping Process cgroup Info") > 0 { capturing_cgroup = 1; next }
capturing_cgroup && (index($0, "End of Process cgroup") > 0 || index($0, "********") > 0) { capturing_cgroup = 0 }
capturing_cgroup {
    cg_line = $0
    gsub(/^[ \t]+|[ \t]+$/, "", cg_line)
    
    if (index(cg_line, "memory.max") > 0) { cg_target = "mem_max"; next }
    if (index(cg_line, "memory.current") > 0) { cg_target = "mem_cur"; next }
    if (index(cg_line, "pids.current") > 0) { cg_target = "pids_cur"; next }
    if (index(cg_line, "pids.max") > 0) { cg_target = "pids_max"; next }
    
    if (cg_target != "" && cg_line != "" && index(cg_line, "Start contents") == 0 && index(cg_line, "End contents") == 0) {
        if (cg_target == "mem_max")  { cgroup_mem_max = cg_line; cg_target = ""; }
        else if (cg_target == "mem_cur")  { cgroup_mem_cur = cg_line + 0; cg_target = ""; }
        else if (cg_target == "pids_cur") { cgroup_pids_cur = cg_line + 0; cg_target = ""; }
        else if (cg_target == "pids_max") { cgroup_pids_max = cg_line; cg_target = ""; }
    }
}

# --- Pass 0.5: Capture System Environment Variables ---
...
END {
    ...
    # Human-Readable Cgroup Summary & Issue Diagnosis
    if (cgroup_mem_cur > 0 || cgroup_pids_cur > 0) {
        print "-- ----------------------------------------------------"
        print "-- Container / Cgroup Resource Usage & Health Diagnosis:"
        print "-- ----------------------------------------------------"
        
        mem_gb = sprintf("%.2f", cgroup_mem_cur / (1024 * 1024 * 1024))
        print "REM   - Current Memory Consumption : " mem_gb " GB (Limit: " (cgroup_mem_max != "" ? cgroup_mem_max : "unlimited") ")"
        print "REM   - Active Processes / Threads : " cgroup_pids_cur " (Limit: " (cgroup_pids_max != "" ? cgroup_pids_max : "unlimited") ")"
        
        # Health Evaluation
        print "REM   --------------------------------------------------"
        if (cgroup_mem_max == "max" && cgroup_pids_max == "max") {
            print "REM   [HEALTH STATUS]: SAFE. No hard cgroup constraints or limits enforced."
        } else {
            print "REM   [HEALTH STATUS]: CONSTRAINED. Hard cgroup caps are active."
        }
        print ""
    }
